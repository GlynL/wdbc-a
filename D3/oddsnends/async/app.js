...
.defer(d3.csv, './simplemaps-worldcities-basic.csv', row => {
  if (Number(row.pop) < 10000) return; /* remove from data */
  return {
    cityName: row.city,
...
.await(function(error, countries, cities) {
  ...
  var countrySelection = d3.select('body').selectAll('div').data(data).enter().append('div');

  countrySelection.append('text').text(d => d.countryName);
  countrySelection.append('ul').html(d => d.cities.map(city => {
